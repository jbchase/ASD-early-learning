function llh = a0b2s_hybrid_llh(theta,data)

beta = theta(1);
alpha = [1e-6 theta(2)];%.2;
stick = [0; theta(3); theta(4); theta(4)];
epsilon = 1e-6;
lapse = theta(5);
ret = theta(6);
bias = theta(7);
...
% state 2: RL
% state 1: biased
% lapse: probability of lapsing from RL to biased
% ret: probability of returning from biased to RL
