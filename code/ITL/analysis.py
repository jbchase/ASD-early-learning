...
def run_analysis(metadata_path: str, csv_folder: str, output_folder: str) -> None:
    ...
    valid = trial_level.dropna(subset=["itl"]).copy()

    # Upper one-tail cuts computed by session only
    session_thresholds = (
        valid.groupby("session")["itl"]
        .quantile([p / 100 for p in PERCENTILES])
        .unstack()
        .rename(columns={p / 100: f"threshold_p{p}" for p in PERCENTILES})
        .reset_index()
    )

    # Upper one-tail cuts computed by session AND genotype
    session_genotype_thresholds = (
        valid.groupby(["session", "genotype"])["itl"]
        .quantile([p / 100 for p in PERCENTILES])
        .unstack()
        .rename(columns={p / 100: f"threshold_p{p}" for p in PERCENTILES})
        .reset_index()
    ...
    valid = valid.merge(session_thresholds, on="session", how="left")
    for p in PERCENTILES:
        valid[f"keep_session_p{p}"] = valid["itl"] <= valid[f"threshold_p{p}"]

    valid = valid.merge(
        session_genotype_thresholds,
        on=["session", "genotype"],
        how="left",
        suffixes=("", "_session_genotype"),
    )
    for p in PERCENTILES:
        valid[f"keep_session_genotype_p{p}"] = (
            valid["itl"] <= valid[f"threshold_p{p}_session_genotype"]
        )

    all_thresholds = pd.concat(
    [session_thresholds, session_genotype_thresholds],
    ignore_index=True,
    sort=False
    )
    ...
    summary_rows = []
    for cut_scope, keep_prefix in [("session", "keep_session_p"), ("session_genotype", "keep_session_genotype_p")]:
        for p in PERCENTILES:
    ...
    main_analysis = animal_summary_long[
        (animal_summary_long["cut_scope"] == "session_genotype") &
        (animal_summary_long["percentile"] == 95)
    ].copy()

    main_analysis.to_csv(output_dir / "animal_summary_sess_geno_p95.csv", index=False)
